...
inherit dist-kernel-utils linux-info mount-boot python-any-r1 savedconfig rhel9
	...
	if use compress-xz || use compress-zstd ; then
		...
		if kernel_is -ge 5 14; then
			...
			if use compress-zstd; then
				eerror "Kernels <5.14 do not support ZSTD-compressed firmware files"
			fi
		fi
	fi
