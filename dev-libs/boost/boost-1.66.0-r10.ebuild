...
IUSE="bzip2 context debug doc icu +nls mpi python static-libs +threads tools"
REQUIRED_USE="
	mpi? ( threads )
	...
	if python_bindings_needed; then
		...
		append_to_user_config() {
			...
			if tc-is-cross-compiler; then
				py_config="using python : ${EPYTHON#python} : : ${ESYSROOT}/usr/include/${EPYTHON}${PYTHON3_ABIFLAGS} : : : : ${PYTHON3_ABIFLAGS} : ${ESYSROOT}/usr/$(get_libdir) ;"
			else
				py_config="using python : ${EPYTHON#python} : ${PYTHON} : $(python_get_includedir)${PYTHON3_ABIFLAGS} : : : : ${PYTHON3_ABIFLAGS} ;"
			fi
...
src_configure() {
	...
	export CFLAGS="$CFLAGS -fno-strict-aliasing -Wno-unused-local-typedefs -Wno-deprecated-declarations"
