...
inherit multilib multilib-minimal multiprocessing preserve-libs cs9
...
if [[ ${PV} == 9999 ]] ; then
	...
	[[ -z ${_CS_ECLASS} ]] || SRC_URI="mirror://openssl/source/${MY_P}.tar.gz"
...
src_configure() {
	...
	append-flags $(test-flags-CC -Wa,--noexecstack) -Wa,--generate-missing-build-notes=yes -DPURIFY
...
multilib_src_configure() {
	...
	local myeconfargs=(
		...
		$(use fips && echo '-DDEVRANDOM="\"/dev/urandom\"" -DREDHAT_FIPS_VERSION="\"3.0.7-023550348ce25675\""')
...
multilib_src_install_all() {
	...
	dodir ${_sysconfdir}/pki/tls
	dosym /etc/ssl/certs /etc/pki/tls/certs
	insinto ${_pkgdocdir}/
	...
	mv ${ED}${_sysconfdir}/ssl/misc/*.pl ${ED}${_bindir}
	mv ${ED}${_sysconfdir}/ssl/misc/tsget ${ED}${_bindir}
	...
	rm -f ${ED}${_sysconfdir}/pki/tls/fipsmodule.cnf
	...
	#endif' ${ED}${_prefix}/include/openssl/opensslconf.h

	if use amd64; then
		insinto ${_prefix}/include/openssl/
		newins "${WORKDIR}"/configuration-prefix.h configuration-${basearch}.h

		cat ${ED}${_prefix}/include/openssl/configuration.h >> \
			${ED}${_prefix}/include/openssl/configuration-${basearch}.h

		insinto ${_prefix}/include/openssl/
		newins "${WORKDIR}"/configuration-switch.h configuration.h
	fi
}
