...
inherit autotools flag-o-matic multilib rhel8-a
...
SRC_URI="${REPO_URI}/${MY_PF}.module_el8.4.0+848+83b90ba8.src.rpm"

LICENSE="|| ( Ruby-BSD BSD-2 )"
KEYWORDS="~alpha amd64 arm arm64 hppa ~mips ppc ppc64 ~riscv ~s390 sparc x86 ~amd64-linux ~x86-linux ~ppc-macos ~x64-macos ~sparc-solaris ~sparc64-solaris ~x64-solaris ~x86-solaris"
IUSE="berkdb debug doc examples gdbm ipv6 jit +rdoc rubytests socks5 +ssl static-libs systemtap tk xemacs"
...
src_compile() {
	emake V=1 EXTLDFLAGS="${LDFLAGS}" MJIT_CFLAGS="${CFLAGS}" MJIT_OPTFLAGS="" MJIT_DEBUGFLAGS="" COPY="cp -p" Q=
}
...
src_install() {
	...
	rm -rf lib/rdoc/rdoc.gemspec || die
	...
	rm -rf "${ED}/usr/bin/"{bundle,bundler,ri,rdoc}"${MY_SUFFIX}" lib/bundler* || die "rm rdoc bins failed"
