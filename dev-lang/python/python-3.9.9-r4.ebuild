...
IUSE="bluetooth build examples gdbm hardened lto ipv6 +ncurses pgo +readline +sqlite +ssl test tk wininst +xml"
RESTRICT="!test? ( test )"
...
src_unpack() {
	rpm_src_unpack ${A}
	mv ${WORKDIR}/{"00001-rpath.patch","00111-no-static-lib.patch","00189-use-rpm-wheels.patch"} ${WORKDIR}/${PATCHSET}/
	rm ${WORKDIR}/${PATCHSET}/0001-Install-libpythonX.Y.a-in-usr-lib-instead-of-usr-lib.patch
	rm ${WORKDIR}/${PATCHSET}/0013-bpo-43998-Default-to-TLS-1.2-and-increase-cipher-sui.patch
}
...
src_compile() {
	...
	emake CFLAGS_NODIST="$CFLAGS_NODIST" regen-all
	emake CFLAGS_NODIST="$CFLAGS_NODIST"
