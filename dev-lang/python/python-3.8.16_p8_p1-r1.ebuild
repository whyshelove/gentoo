...
IUSE="
	bluetooth build examples gdbm hardened lto +ncurses pgo
...
src_prepare() {
	...
	rm -r "${WORKDIR}/${PATCHSET}"/000{1,2}-*.patch
	rm -r "${WORKDIR}/${PATCHSET}"/002{1,2,3,5,8}-*.patch
...
src_configure() {
	...
	local -x LDFLAGS_NODIST="$LDFLAGS -g $(pkg-config --libs-only-L openssl)"
