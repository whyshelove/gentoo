...
PYVER=$(ver_cut 1-2)
PATCHSET="python-gentoo-patches-${MY_PV}_p8"
...
IUSE="
	bluetooth build ensurepip examples gdbm hardened ipv6 lto +ncurses pgo
...
src_prepare() {
	...
	rm -r "${WORKDIR}/${PATCHSET}"/000{1,2,3}-*.patch
	rm -r "${WORKDIR}/${PATCHSET}"/002{2,3,4,5}-*.patch
...
src_configure() {
	...
	export LDFLAGS_NODIST="$LDFLAGS -g $(pkg-config --libs-only-L openssl)"

	filter-flags -malign-double '-specs=*'
...
src_compile() {
	...
	# in _sysconfigdata*
	emake CFLAGS_NODIST="$CFLAGS_NODIST" regen-all PYTHON_FOR_REGEN="python3.8"
