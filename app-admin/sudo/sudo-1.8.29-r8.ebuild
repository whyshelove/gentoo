...
src_configure() {
	append-cflags -fpie
	append-ldflags -pie -Wl,-z,relro -Wl,-z,now
	local SECURE_PATH
	set_secure_path
	...
	myeconfargs=(
		...
		$(use_with ldap)
		$(use_with ldap ldap_conf_file /etc/sudo-ldap.conf)
...
src_install() {
	...
	sed -i 's|/etc/ldap\.conf|/etc/sudo-ldap.conf|g' README.LDAP
	rm -rf "${ED}"${_mandir}/man5/sudo-ldap.conf.5 || die

	if use ldap ; then
		dodoc README.LDAP

		cat <<-EOF > "${WORKDIR}"/sudo-ldap.conf
		...
		insinto /etc
		doins -r "${WORKDIR}"/{"sudo-ldap.conf","sudoers","sudo.conf"}
		fperms -R 0640 /etc/{"sudo-ldap.conf","sudo.conf"}
	...
	if use pam; then
		...
		pamd_mimic sudo sudo-i auth account password session
...
pkg_postinst() {
	...
	if use ldap ; then
		ewarn
		ewarn "sudo uses the /etc/sudo-ldap.conf file for ldap configuration."
		ewarn
