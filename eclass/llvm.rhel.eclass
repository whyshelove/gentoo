...
# @ECLASS: llvm.rhel.eclass
# @MAINTAINER:
# Michał Górny <mgorny@gentoo.org>
# @AUTHOR:
# Michał Górny <mgorny@gentoo.org>
# @SUPPORTED_EAPIS: 7 8
# @BLURB: Common bits for fetching & unpacking llvm.rhel projects
# @DESCRIPTION:
# The llvm.rhel eclass provides common code to fetch and unpack parts
# of the llvm.rhel project tree.  It takes care of handling both git
...
# LLVM_COMPONENTS and then call llvm.rhel_set_globals.  If tests require
...
# @INTERNAL
# @DESCRIPTION:
# Source type to use: 'git' or 'rpm'.
if [[ -z ${_LLVM_SOURCE_TYPE+1} ]]; then
	if [[ ${PV} == *.9999 ]]; then
		_LLVM_SOURCE_TYPE=git
	else
		_LLVM_SOURCE_TYPE=rpm
	fi
fi

[[ ${_LLVM_SOURCE_TYPE} == git ]] && inherit git-r3

[[ ${PV} == ${_LLVM_MASTER_MAJOR}.* && ${_LLVM_SOURCE_TYPE} == rpm ]] &&
	die "${ECLASS}: Release ebuild for master branch?!"

inherit multiprocessing flag-o-matic rhel8-a
...
# @FUNCTION: llvm.rhel_set_globals
# @DESCRIPTION:
...
llvm.rhel_set_globals() {
	...
	if [[ ${_LLVM_SOURCE_TYPE} == git ]]; then
		...
		case ${PN} in
		    llvm) COMMIT=937+1cafe22c ;;
		    clang|libomp|compiler-rt) COMMIT=937+1cafe22c ;;
		    *)  ;;
		esac

		SRC_URI="${REPO_URI}/${MY_PF}.module_el8.6.0+${COMMIT}.src.rpm"
	else
...
# @FUNCTION: llvm.rhel_src_unpack
# @DESCRIPTION:
# Unpack or checkout requested LLVM components.
llvm.rhel_src_unpack() {
	if [[ ! ${_LLVM_ORG_SET_GLOBALS_CALLED} ]]; then
		die "llvm.rhel_set_globals must be called in global scope"
	fi
...
# @FUNCTION: llvm.rhel_src_prepare
# @DESCRIPTION:
