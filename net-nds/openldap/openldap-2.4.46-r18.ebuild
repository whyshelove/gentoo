...
inherit autotools db-use flag-o-matic multilib-minimal preserve-libs ssl-cert toolchain-funcs systemd tmpfiles rhel8
...
if [[ ${PV} != *8888 ]]; then
	SRC_URI="${SRC_URI} mirror://gentoo/${BIS_P}"
	pwdChecker_lib="${S}/ltb-project-openldap-ppolicy-check-password-${check_password_version}"
	S=$S/$P
fi
...
src_prepare() {
	default
	[[ ${PV} == *8888 ]] && AUTOMAKE=/usr/bin/true eautoreconf -fi
}
...
multilib_src_configure() {
	...
	if ! use minimal && multilib_is_native_abi; then
		...
		for backend in backends dnssrv ldap mdb meta monitor null passwd relay shell sock; do
...
multilib_src_compile() {
	...
	# for head in portable lber_types ldap_config ldap_features;
	# do
	# 	ln -s $S/include/${head}.hin $S/include/${head}.h
	# done

	# pushd ${pwdChecker_lib}
	# 	emake LDAP_INC="-I../${P}/include \
	# 	-I../${P}/servers/slapd \
 	# 	-I../${P}/build-servers/include"
	# popd
}
...
multilib_src_install() {
	...
	# 	ln -s check_password.so.${check_password_version} "${D}"/usr/lib64/openldap/check_password.so
	# 	install -m 755 check_password.so.${check_password_version} "${D}"/usr/lib64/openldap/
	# 	# install -m 644 README "${D}"/usr/lib64/openldap
	# 	install -d -m 755 "${D}"/etc/openldap
	# 	cp "${FILESDIR}"/check_password.conf "${D}"/etc/openldap/
	# 	mv README{,.check_pwd}
	# popd
