...
inherit flag-o-matic python-r1 toolchain-funcs multilib-minimal rhel9
...
multilib_src_compile() {
	...
	emake \
		LIBDIR="\$(PREFIX)/$(get_libdir)" swigify
	...
	if multilib_is_native_abi && use ruby; then
		building() {
			...
			emake \
				RUBY=${1} \
				RUBYINC="$(pkg-config --cflags ruby)" \
	...
	if multilib_is_native_abi && use python; then
		installation() {
			emake DESTDIR="${D}" \
				...
				LIBSEPOLA="\$(PREFIX)${_libdir}/libsepol.a" \
				install-pywrap
