...
inherit autotools multilib-minimal systemd rhel9
...
multilib_src_configure() {
	ECONF_SOURCE="${S}" econf \
		...
		$(use_with trust trust-paths "${EPREFIX}"/etc/ssl/certs/ca-certificates.crt:"${EPREFIX}"/etc/pki/ca-trust/source:"${EPREFIX}"${_datadir}/pki/ca-trust-source) \
		$(use_enable debug) \
		$(use_with libffi) \
...
multilib_src_install_all() {
	...
	dodir ${_sysconfdir}/pkcs11/modules ${_docdir}/${PN}

	insinto ${_libexecdir}/p11-kit/ && insopts -m0755
	doins "${WORKDIR}"/trust-extract-compat

	# Install the example conf with $$doc instead
	mv ${ED}${_sysconfdir}/pkcs11/pkcs11.conf.example ${ED}${_docdir}/${PN}/pkcs11.conf.example
