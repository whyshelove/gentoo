...
inherit bash-completion-r1 elisp-common python-any-r1 toolchain-funcs rhel9-c
...
src_test() {
	if ! tc-is-cross-compiler; then
		...
		./ninja_test --gtest_filter=-SubprocessTest.SetWithLots || die
	fi
}
...
src_install() {
	...
	install -Dpm0644 ${WORKDIR}/macros.ninja ${D}/usr/lib/rpm/macros.d/macros.ninja

	# Macro should not change when we are redefining bindir
	sed -i -e "/^%%__ninja /s| .*$| ${D}/usr/bin/ninja|" ${D}/usr/lib/rpm/macros.d/macros.ninja
