...
IUSE="abi_mips_n32 audit bindist debug hpn kerberos ldns libedit livecd pam +pie +scp sctp security-key selinux +ssl static test X X509 xmss"
...
src_configure() {
	...
	if use kerberos ; then
		...
		append-cflags -I${_includedir}/gssapi
	fi
	...
	local myconf=(
		...
		--with-privsep-path="${EPREFIX}"/var/empty.sshd
...
src_install() {
	...
	keepdir	${_datadir}/empty.sshd /var/empty.sshd

	systemd_dounit "${WORKDIR}"/sshd.{service,socket}
	systemd_dounit "${WORKDIR}"/'sshd@.service'

	systemd_dounit "${WORKDIR}"/sshd-keygen.target
	systemd_dounit "${WORKDIR}"/'sshd-keygen@.service'
	systemd_douserunit "${WORKDIR}"/ssh-agent.service
}
