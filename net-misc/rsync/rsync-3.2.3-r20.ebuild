...
src_install() {
	emake DESTDIR="${D}" install INSTALLCMD='install -p' INSTALLMAN='install -p'
	...
	newins "${WORKDIR}"/rsyncd.conf rsyncd

	systemd_dounit "${WORKDIR}"/rsyncd.{service,socket}
	systemd_dounit "${WORKDIR}"/'rsyncd@.service'
}
