...
[[ ${suffix_ver} ]] && DSUFFIX="_${suffix_ver}.2"
...
multilib_src_configure() {
	...
	if use ssl ; then
		...
		if use openssl || use curl_ssl_openssl; then
			...
			myconf+=( --with-ssl --with-ca-path="${EPREFIX}"/etc/ssl/certs --with-ca-bundle="${EPREFIX}"${_sysconfdir}/pki/tls/certs/ca-bundle.crt )
		fi
