...
inherit autotools lua-single ssl-cert toolchain-funcs perl-module flag-o-matic user systemd multilib pax-utils rhel8-a

DESCRIPTION="Robust, small and high performance http and reverse proxy server"
HOMEPAGE="https://nginx.org"
SRC_URI="${REPO_URI}/${MY_PF}.module_el8.5.0+899+43b718f6.src.rpm
...
src_configure() {
	...
	./configure \
		--prefix="${EPREFIX}"${_datadir}/nginx \
		--sbin-path="${EPREFIX}"${_sbindir}/nginx \
		--modules-path="${EPREFIX}"${_libdir}/nginx/modules \
		--conf-path="${EPREFIX}"/etc/${PN}/${PN}.conf \
		--error-log-path="${EPREFIX}"/var/log/${PN}/error_log \
		--pid-path="${EPREFIX}"/run/${PN}.pid \
		--lock-path="${EPREFIX}"/run/lock/subsys/${PN}.lock \
		--with-cc-opt="-I${ESYSROOT}/usr/include${WITHOUT_IPV6} $CFLAGS $(pcre-config --cflags)" \
		--with-ld-opt="-L${ESYSROOT}/usr/$(get_libdir) $LDFLAGS -Wl,-E" \
		--http-log-path="${EPREFIX}"/var/log/${PN}/access.log \
		--http-client-body-temp-path="${EPREFIX}${NGINX_HOME_TMP}"/client_body \
...
src_install() {
	...
	cp ./nginx.conf "${ED}"/etc/nginx/nginx.conf || die

	newinitd "${FILESDIR}"/nginx.initd-r4 nginx
	newconfd "${FILESDIR}"/nginx.confd nginx

	systemd_dounit ./nginx.service

	exeinto ${_bindir}
	doexe "${WORKDIR}"/nginx-upgrade
	doman man/nginx.8 "${WORKDIR}"/nginx-upgrade.8
