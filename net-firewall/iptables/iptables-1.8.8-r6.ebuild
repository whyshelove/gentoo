...
DSUFFIX="_1"
inherit systemd toolchain-funcs autotools flag-o-matic usr-ldscript rhel9
...
src_configure() {
	...
	local myeconfargs=(
		...
		--enable-ipv6
		--enable-shared
		--with-kernel="${EPREFIX}"/usr
		--with-kbuild="${EPREFIX}"/usr
		--with-ksource="${EPREFIX}"/usr
		$(use_enable nftables)
...
src_install() {
	...
	script_path=${_libexecdir}/iptables
	legacy_actions=${_libexecdir}/initscripts/legacy-actions
	S1=${WORKDIR}/iptables.init
	S2=${WORKDIR}/iptables-config
	S3=${WORKDIR}/iptables.service

	diropts -m 0755 && dodir ${script_path} ${_sysconfdir}/sysconfig

	sed -e 's;iptables;ip6tables;g' -e 's;IPTABLES;IP6TABLES;g' < ${S1} > ip6tables.init
	insinto ${script_path} && insopts -m0755
	doins -r $S1 ip6tables.init

	sed -e 's;iptables;ip6tables;g' -e 's;IPTABLES;IP6TABLES;g' < ${S2} > ip6tables-config
	insinto ${_sysconfdir}/sysconfig && insopts -m0600
	doins -r $S2 ${WORKDIR}/{"ebtables-config","sysconfig_iptables","sysconfig_ip6tables"} ip6tables-config

	sed -e 's;iptables;ip6tables;g' -e 's;IPv4;IPv6;g' -e 's;/usr/libexec/ip6tables;/usr/libexec/iptables;g' < ${S3} > ip6tables.service
	# install systemd service files
	systemd_dounit $S3 ip6tables.service ${WORKDIR}/{"arptables.service","ebtables.service"}
