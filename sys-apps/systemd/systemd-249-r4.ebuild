...
multilib_src_install_all() {
	...
	dodir ${_localstatedir}/{lib,log,cache}/private ${_localstatedir}/lib/private/systemd/journal-upload
	...
	insinto /etc/pam.d/
	doins "${WORKDIR}"/systemd-user

	insinto ${_pkgdocdir}/
	doins "${WORKDIR}"/20-yama-ptrace.conf
	...
	insopts -m0755 && doins "${WORKDIR}"/20-grubby.install

	insinto ${_prefix}/lib/systemd/
	doins "${WORKDIR}"/libsystemd-shared.abignore

	# systemd-oomd default configuration
	insinto ${_prefix}/lib/systemd/oomd.conf.d/
	doins "${WORKDIR}"/10-oomd-defaults.conf


	insinto ${system_unit_dir}/'-.slice.d'/
	doins "${WORKDIR}"/10-oomd-root-slice-defaults.conf

	insinto ${system_unit_dir}/'user@.service.d'/
	doins "${WORKDIR}"/10-oomd-user-service-defaults.conf

	insinto ${_rpmconfigdir}/macros.d/
	doins "${WORKDIR}"/macros.sysusers

	insinto ${_rpmconfigdir}/fileattrs/
	doins ${WORKDIR}/sysusers.attr

	exeinto ${_rpmconfigdir}/
	doexe ${WORKDIR}/{"sysusers.prov","sysusers.generate-pre.sh"}
	...
	keepdir /var/lib/systemd/{coredump,backlight,rfkill,linger}
