...
SLOT="0/2"
IUSE="acl apparmor audit build cgroup-hybrid cryptsetup curl dns-over-tls elfutils +gcrypt gnuefi homed http +hwdb idn importd +kmod +lz4 lzma nat pam pcre pkcs11 policykit pwquality qrcode repart +resolvconf +seccomp selinux split-usr static-libs +sysv-utils test tpm vanilla xkb"
...
src_prepare() {
	...
	sed -i "/%meson_install/d" ${WORKDIR}/*.spec
	default
}
...
multilib_src_configure() {
	local myconf=(
		...
		# no deps
		-Dima=true
		-Ddefault-hierarchy=$(usex cgroup-hybrid hybrid unified legacy)
...
multilib_src_install_all() {
	...
	keepdir ${_localstatedir}/lib/systemd/{coredump,backlight,rfkill,linger}

	dodir ${_localstatedir}/lib/private ${_localstatedir}/lib/private/systemd/journal-upload
	...
	doexe ${WORKDIR}/rc.local
	dosym rc.d/rc.local ${_sysconfdir}/rc.local
