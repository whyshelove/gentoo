...
src_configure() {
	...
	myeconfargs=(
		...
		--sysconfdir=${EPREFIX}${_sysconfdir}/$PN/
		--with-systemdenvfile=${EPREFIX}${_sysconfdir}/sysconfig/$PN
...
src_install() {
	...
	newins "${WORKDIR}"/smartmontools.sysconf smartmontools

	exeinto ${_libexecdir}/${PN}/
	doexe "${WORKDIR}"/smartdnotify
