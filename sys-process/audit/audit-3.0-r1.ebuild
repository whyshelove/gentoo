...
if [[ ${PV} != *8888 ]]; then
	MY_PR=${PVR##*r}
	MY_PF=${P}-0.17.20191104git1c2f876
fi
...
lockdown_perms() {
	...
	local basedir="$1"
	chmod 0755 "${basedir}"/sbin/au{ditctl,ditd,report,search,trace} 2>/dev/null
	chmod 0750 "${basedir}"/var/log/audit 2>/dev/null
	chmod 0750 "${basedir}"/etc/audit/{auditd.conf,audit*.rules*} 2>/dev/null
}
